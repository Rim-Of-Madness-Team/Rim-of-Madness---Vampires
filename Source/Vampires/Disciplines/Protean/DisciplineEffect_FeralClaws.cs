...
namespace Vampire
{
    public class DisciplineEffect_FeralClaws : Verb_UseAbilityPawnEffect
    {
        public override void Effect(Pawn target)
        {
            ...
            if (recs.FirstOrDefault(x => x.def == BodyPartDefOf.Hand) is BodyPartRecord leftHand)
                bodyPartRecords.Add(leftHand, VampDefOf.ROMV_FeralClaw);
            if (recs.FirstOrDefault(x => x.def == BodyPartDefOf.Hand) is BodyPartRecord rightHand)
                bodyPartRecords.Add(rightHand, VampDefOf.ROMV_FeralClaw);
            // Might want to check this Jeccy, changed LeftHand and RightHand to just Hand because it no longer exists
