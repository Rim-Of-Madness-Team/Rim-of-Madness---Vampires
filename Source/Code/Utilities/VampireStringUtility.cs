        ...
        try
        {
            if (generation != -1)
                s.AppendLine(hediffGeneration.LabelCap +
                             " (" +
                             "ROMV_HI_Generation".Translate(AddOrdinal(generation)) +
                             ")");
            else
                s.AppendLine(hediffGeneration.LabelCap);
            var painFactor = hediffGeneration.stages[0].painFactor.ToStringPercent();
            var sensesFactor = hediffGeneration.stages[0].capMods.First().offset.ToStringPercent();
            ...
            if (!hediffGeneration.comps.NullOrEmpty())
                foreach (var compProps in hediffGeneration.comps)
                {
