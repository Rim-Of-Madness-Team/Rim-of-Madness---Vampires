...
public static class BloodUtility
{
    ...
    public static bool TryFindBestBloodSourceFor(Pawn getter, Pawn eater, bool desperate, out Thing bloodSource,
        out ThingDef bloodDef, bool canUseInventory = true, bool allowForbidden = false)
    {
        ...
        var thing2 = BestBloodSourceOnMap(getter, eater, desperate, BloodPreferabilty.Highblood, allowForbidden);
        if (thing == null && thing2 == null)
        {
            ...
            if (getter == eater)
            {
                var pawn = BestPawnToHuntForVampire(getter);
                if (pawn != null)
                {
                    bloodSource = pawn;
                    bloodDef = pawn.def;
    ...
    public static Thing BestBloodInInventory(Pawn holder, Pawn eater = null, int minBloodPointsRequested = 1)
    {
        ...
        for (var i = 0; i < innerContainer.Count; i++)
        {
            ...
            if (thing.TryGetComp<CompBloodItem>() is { } c)
    ...
    // RimWorld.FoodUtility
    public static Thing BestBloodSourceOnMap(Pawn getter, Pawn eater, bool desperate,
        BloodPreferabilty maxPref = BloodPreferabilty.Highblood, bool allowForbidden = false)
    {
        ...
        {
            Log.Error(string.Concat(getter, " tried to find blood to bring to ", eater, " but ", getter,
                " is incapable of Manipulation."));
            return null;
        }

        var minPref = eater?.VampComp()?.Bloodline?.minBloodPref ?? BloodPreferabilty.Any;
        if (desperate) minPref = eater?.VampComp()?.Bloodline?.desperateBloodPref ?? BloodPreferabilty.Any;
        Predicate<Thing> foodValidator = delegate(Thing t)
        {
            if (t.TryGetComp<CompBloodItem>() is { } bl)
            {
                if (!allowForbidden && t.IsForbidden(getter)) return false;
                if ((int)bl.Props.bloodType < (int)minPref) return false;
                if ((int)bl.Props.bloodType > (int)maxPref) return false;
                if (t.IsBurning() || (!desperate && t.IsNotFresh()) || !getter.CanReserve(t)) return false;
                return true;
            }
        ...
        thing = SpawnedBloodItemScan(eater, getter.Position,
            getter.Map.listerThings.ThingsInGroup(ThingRequestGroup.FoodSourceNotPlantOrTree), PathEndMode.ClosestTouch,
            TraverseParms.For(getter), 9999f, validator);
        return thing;
    }
    ...
    public static float GetPreyScoreFor(Pawn predator, Pawn prey)
    {
        ...
        if (prey.Downed) num2 = Mathf.Min(num2, 0.2f);
        ...
        if (predator?.BloodNeed() is Need_Blood vampBlood &&
            ...
            switch (vampBlood.preferredFeedMode)
            {
                ...
                case PreferredFeedMode.AnimalNonLethal:
                    if (!victimBlood.DrainingIsDeadly && victimBlood.CurBloodPoints > 1 && prey.RaceProps.Animal)
                        num3 += GetAnimalHuntScore(predator, prey);
                    break;
            }
            ...
            if (vampire?.BloodNeed() is Need_Blood eaterBlood)
            {
                ...
                if (victim.RaceProps.Animal)
                {
                    ...
                    var firstDirectRelationPawn =
                        victim.relations.GetFirstDirectRelationPawn(PawnRelationDefOf.Bond, x => !x.Dead);
                    if (firstDirectRelationPawn != null) return false;
                }
    ...
    {
        var num = Mathf.Min(10, StackCountForBlood(def, ingester.VampComp().BloodPool.BloodWanted));
        if (num < 1) num = 1;
        return num;
    }
    ...
    {
        if (def.GetCompProperties<CompProperties_BloodItem>() is { } bloodItem)
            return Mathf.Max(
                Mathf.RoundToInt(nutrition / def.GetCompProperties<CompProperties_BloodItem>().bloodPoints), 1);
        return 0;
    }
}