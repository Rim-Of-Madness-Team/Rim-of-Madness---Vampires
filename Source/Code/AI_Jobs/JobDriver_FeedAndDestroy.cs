...
public class JobDriver_FeedAndDestroy : JobDriver
{
    ...
    private readonly int bloodCarried = 0;
    ...
    private void DoEffect()
    {
        ...
        if (Victim.health.hediffSet.GetNotMissingParts().ToList().FindAll(x => x.depth == BodyPartDepth.Inside) is { } parts)
            for (var i = 0; i < 3; i++)
                if (!Victim.Dead)
                    Victim.TakeDamage(new DamageInfo(DamageDefOf.Bite, Rand.Range(8, 12), 1f, -1, GetActor(),
                        parts.RandomElement()));
    }
