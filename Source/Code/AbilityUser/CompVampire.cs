...
namespace Vampire
{
    ...
    public class CompVampire : CompAbilityUser
    {
        public void Notify_DeGhouled()
        {
            ...
            Find.LetterStack.ReceiveLetter("ROMV_LostGhoulPowersTitle".Translate(), contents,
                VampDefOf.ROMV_GoodMessage, AbilityUser);
        }
        ...
        private SunlightPolicy
            curSunlightPolicy =
                SunlightPolicy.Restricted; //This is dummy code no longer used. Kept for save file purposes.
        ...
        public bool IsRevenant =>
            IsGhoul &&
            (AbilityUser?.health?.hediffSet
                 ?.GetFirstHediffOfDef(VampDefOf.ROM_GhoulHediff) is HediffGhoul hediffGhoul &&
             hediffGhoul.ghoulType == GhoulType.Revenant);
        ...
        public void Notify_LevelUp(bool sendNotification)
        {
            ...
            {
                string contents = (IsVampire)
                    ? "ROMV_LevelUp".Translate(AbilityUser)
                    : "ROMV_LevelUpGhoul".Translate(AbilityUser);
                string contentsLetter =
                    ((IsVampire) ? "ROMV_LevelUp".Translate(AbilityUser) : "ROMV_LevelUpGhoul".Translate(AbilityUser)) +
                    " " + "ROMV_LevelUpMoreInfo".Translate();
                Messages.Message(contents,
                    new GlobalTargetInfo(AbilityUser),
                    DefDatabase<MessageTypeDef>.GetNamed("ROMV_VampireNotifaction"));
                Find.LetterStack.ReceiveLetter("ROMV_LevelUpShort".Translate(), contentsLetter,
                    VampDefOf.ROMV_LevelUpMessage, AbilityUser);
            }
        }
            ...
            bool firstVampire = false)
        {
            ...
            Find.World.GetComponent<WorldComponent_VampireTracker>().AddVampire(AbilityUser, newSire, bloodline,
                generation, AbilityUser.ageTracker.AgeBiologicalYearsFloat);
        }
        ...
        public void SunlightWatcherTick()
        {
            if (VampireSettings.Get.aiToggle && Find.TickManager.TicksGame % 250 == 0 &&
                GetSunlightPolicy(Pawn) != SunlightPolicy.NoAI)
            {
                try
                {
        ...
        public void Notify_Embraced(CompVampire sireComp)
        {
            ...
            Messages.Message(contents, MessageTypeDefOf.SilentInput);
            LookTargets targets = sireComp?.AbilityUser == null
                ? new LookTargets(AbilityUser)
                : new LookTargets(new List<Pawn> { AbilityUser, sireComp.AbilityUser });
            Find.LetterStack.ReceiveLetter("ROMV_NewVampire".Translate(), contents, VampDefOf.ROMV_GoodMessage,
                targets);
        }
        ...
        public void Notify_Diablerie(CompVampire victim)
        {
            ...
            Messages.Message(contents, MessageTypeDefOf.SilentInput);
            Find.LetterStack.ReceiveLetter("ROMV_DiablerieSuccess".Translate(), contents, VampDefOf.ROMV_GoodMessage,
                AbilityUser);
        ...
        public HediffGhoul GhoulHediff =>
            this.Pawn.health.hediffSet.GetFirstHediffOfDef(VampDefOf.ROM_GhoulHediff) as HediffGhoul;
                ...
                if (!(this.AbilityUser?.health?.summaryHealth?.SummaryHealthPercent > 0.99f))
                {
                    ...
                    yield return new Command_Action()
                    {
                        ...
                        action = delegate
                        {
                            ...
                            MoteMaker.ThrowText(AbilityUser.DrawPos, AbilityUser.Map,
                        ...
                        disabled = (AbilityUser.IsGhoul())
                            ? BloodPool.CurGhoulVitaePoints <= 0
                            : BloodPool.CurBloodPoints <= 0
                    };
                }
        ...
        public override void PostExposeData()
        {
            ...
            if (Scribe.mode == LoadSaveMode.PostLoadInit)
            {
                ...
                if ((AbilityUser.IsVampire(false) || AbilityUser.IsGhoul()) &&
                    (AbilityData.Powers == null || AbilityData.Powers.NullOrEmpty()))
                {
