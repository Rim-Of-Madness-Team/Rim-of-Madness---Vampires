...
static partial class HarmonyPatches
{
    ...
    public static void HarmonyPatches_Royalty(Harmony harmony)
    {
        ...
        harmony.Patch(
            AccessTools.Property(typeof(MonumentMarker), nameof(MonumentMarker.FirstDisallowedBuilding)).GetGetMethod(),
            null,
    ...
    public static void VampExceptionsForMonumentMarkers(MonumentMarker __instance, ref Thing __result)
    {
        if (__result?.def?.defName == "ROMV_SarcophagusBed")
        {
            ...
            var terrain = __instance.sketch.Terrain;
            for (var i = 0; i < terrain.Count; i++)
            {
                var tmpAllowedBuildings =
                    (List<ThingDef>)AccessTools.Field(typeof(MonumentMarker), "tmpAllowedBuildings")
                        .GetValue(__instance);
                ...
                if (sketchThing != null) tmpAllowedBuildings.Add(sketchThing.def);
                if (list != null)
                    for (var j = 0; j < list.Count; j++)
                        tmpAllowedBuildings.Add(list[j].def);
                var thingList = (terrain[i].pos + __instance.Position).GetThingList(__instance.Map);
                for (var k = 0; k < thingList.Count; k++)
                    if (thingList[k].def.IsBuildingArtificial && !thingList[k].def.IsBlueprint &&
                        !thingList[k].def.IsFrame && !tmpAllowedBuildings.Contains(thingList[k].def))
                    {
    ...
    public static void VampsCanDrinkBloodWineNotHumans(JoyGiver_SocialRelax __instance, IntVec3 center, Pawn ingester,
        ref Thing ingestible, ref bool __result)
    {
    ...
    private static bool VanillaModified_TryFindIngestibleToNurse(JoyGiver_SocialRelax __instance, IntVec3 center,
        Pawn ingester, out Thing ingestible)
    {
        ...
        for (var i = 0; i < currentPolicy.Count; i++)
            if (currentPolicy[i].allowedForJoy && currentPolicy[i].drug.ingestible.nurseable)
                ((List<ThingDef>)AccessTools.Field(typeof(JoyGiver_SocialRelax), "nurseableDrugs").GetValue(__instance))
                    .Add(currentPolicy[i].drug);
        ((List<ThingDef>)AccessTools.Field(typeof(JoyGiver_SocialRelax), "nurseableDrugs").GetValue(__instance))
            .Shuffle();
        for (var j = 0;
             j < ((List<ThingDef>)AccessTools.Field(typeof(JoyGiver_SocialRelax), "nurseableDrugs")
                 .GetValue(__instance)).Count;
             j++)
        {
            var list = ingester.Map.listerThings.ThingsOfDef(
                ((List<ThingDef>)AccessTools.Field(typeof(JoyGiver_SocialRelax), "nurseableDrugs")
                    .GetValue(__instance))[j]);
            if (list.Count > 0)
            {
                Predicate<Thing> validator = delegate(Thing t)
                {
                    if (t.def?.graphicData?.texPath == "Things/Item/Resource/BloodWine") return false;
                    if (ingester.CanReserve(t)) return !t.IsForbidden(ingester);
                    return false;
                };
                ingestible = GenClosest.ClosestThing_Global_Reachable(center, ingester.Map, list, PathEndMode.OnCell,
                    TraverseParms.For(ingester), 40f, validator);
                if (ingestible != null) return true;
            }
        }
    ...
    {
        foreach (var thing in __instance.things)
    ...
    public static bool Vamp_RestoreTitlesWhenResurrected(Pawn_RoyaltyTracker __instance)
    {
        ...
        if (pawn.IsVampire(true))
        {
            ...
            foreach (var item in titleList) pawn.royalty.SetTitle(item.faction, item.def, false, false, false);
            dict.Remove(pawn);
            return false;
        }
