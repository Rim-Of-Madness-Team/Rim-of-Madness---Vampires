...
{
    public static Dictionary<Pawn, int> VampGuestCache = new();
    ...
    public static void HarmonyPatches_Misc(Harmony harmony)
    {
        ...
        harmony.Patch(AccessTools.Method(typeof(Designator_Slaughter), "CanDesignateThing"),
            new HarmonyMethod(typeof(HarmonyPatches), nameof(Vamp_CantSlaughterTemps)));
        //Log.Message("74");
        ...
        harmony.Patch(AccessTools.Method(typeof(Listing_TreeThingFilter), "Visible", new[] { typeof(ThingDef) }),
            null,
        ...
        harmony.Patch(AccessTools.Method(typeof(PawnAddictionHediffsGenerator),
            ...
            new HarmonyMethod(typeof(HarmonyPatches), nameof(GenerateAddictionsAndTolerancesFor_PreFix)));
        //Log.Message("85");

        harmony.Patch(AccessTools.Method(typeof(JobGiver_PackFood),
                "TryGiveJob"),
            new HarmonyMethod(typeof(HarmonyPatches), nameof(VampsDontPackFood)));
        //Log.Message("86");

        harmony.Patch(AccessTools.Method(typeof(Pawn_DrugPolicyTracker),
                nameof(Pawn_DrugPolicyTracker.ShouldTryToTakeScheduledNow)),
            new HarmonyMethod(typeof(HarmonyPatches), nameof(VampiresDontHaveDrugSchedules)));
        //Log.Message("87");
        ...
        harmony.Patch(AccessTools.Method(typeof(PawnCapacitiesHandler),
            ...
            new HarmonyMethod(typeof(HarmonyPatches), nameof(VampiresDontNeedLungs)));

        // Attempt to patch DeathRattle's AddCustomHediffs method
        try
        {
            harmony.Patch(
                AccessTools.Method(AccessTools.TypeByName("DeathRattle.Harmony.ShouldBeDeadFromRequiredCapacityPatch"),
                    "AddCustomHediffs"),
                new HarmonyMethod(typeof(HarmonyPatches), nameof(VampiresDontNeedLungsDeathRattle)));
        }
        catch
        {
        }
    ...
    private static float AlertNeedWarmClothes_LowestTemperatureComing(Map map)
    {
        ...
        var a = GenTemperature.AverageTemperatureAtTileForTwelfth(map.Tile, twelfth);
        for (var i = 0; i < 3; i++)
        {
    ...
    public static void BestAttackTarget(IAttackTargetSearcher searcher, TargetScanFlags flags,
        ...
        IntVec3 locus, float maxTravelRadiusFromLocus, bool canBashDoors,
        bool canTakeTargetsCloserThanEffectiveMinRange, ref IAttackTarget __result)
    {
        if (searcher?.Thing is Pawn pSearch && __result?.Thing is Pawn p && p.IsVampire(true) &&
            p.VampComp().Sheet.Disciplines.FirstOrDefault(x => x.Def.defName == "ROMV_Presence") is { } d)
        {
            ...
            if (defToApply != null) HealthUtility.AdjustSeverity(pSearch, defToApply, 1.0f);
        }
    }
    ...
    public static void VampLevel(SkillRecord __instance, ref int __result)
    {
        ...
        if (!__instance.TotallyDisabled)
            if (p.health.hediffSet.hediffs.FindAll(x =>
                    x.TryGetComp<HediffComp_SkillOffset>() is { } hSkill &&
                    hSkill.Props.skillDef == __instance.def) is { } hediffs &&
                !hediffs.NullOrEmpty())
        ...
        PawnGenerationContext context)
    {
        ...
        if (VampireUtility.IsDaylight(currentMap) && pawn.Faction != Faction.OfPlayerSilentFail &&
            pawn?.health?.hediffSet?.hediffs is { } hdiffs)
        {
            hdiffs.RemoveAll(x => x.def == VampDefOf.ROM_Vampirism);
            if (recentVampires?.ContainsKey(pawn) ?? false) recentVampires.Remove(pawn);
        }
        else
        {
        ...
        ref float __result)
    {
        ...
        if (VampireSettings.Get.sunDimming > 0f) __result = Mathf.Clamp01(__result - VampireSettings.Get.sunDimming);
    }
    ...
    public static void Vamp_DontNeedWarmClothesReports(Alert_NeedWarmClothes __instance, ref AlertReport __result)
    {
        if (__result.culpritsTargets?.Count() > 0)
        {
            ...
            if (vamps?.Count() > 0)
            {
                ...
                var num = AlertNeedWarmClothes_LowestTemperatureComing(p.MapHeld);
                ...
                if (!colonists.NullOrEmpty())
                    foreach (var pawn in colonists)
                        if (pawn.GetStatValue(StatDefOf.ComfyTemperatureMin) > num)
                        {
                            __result = pawn;
                            return;
                        }
        ...
        get_LetVampiresKeepMapsOpen(MapPawns __instance, ref bool __result) //get_AnyPawnBlockingMapRemoval
    {
        ...
        foreach (var t in list)
        {
            ...
            if (hideyHole != null && hideyHole.ContainedThing is Pawn p && !p.Dead) __result = true;
        }
        ...
        IEnumerable<Pawn> startingPawns, ref Lord __result)
    {
        ...
        foreach (var startingPawn in startingPawns)
            ...
            {
                if (VampGuestCache.ContainsKey(startingPawn)) VampGuestCache.Remove(startingPawn);

                var curTicks = Find.TickManager.TicksGame;
                VampGuestCache.Add(startingPawn, curTicks);
    ...
    public static bool VampiresDontHaveDrugSchedules(Pawn_DrugPolicyTracker __instance, ThingDef ingestible,
        ref bool __result)
    {
