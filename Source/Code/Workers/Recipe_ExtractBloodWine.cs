...
public class Recipe_ExtractBloodWine : Recipe_Surgery
{
    ...
    public override IEnumerable<BodyPartRecord> GetPartsToApplyOn(Pawn pawn, RecipeDef recipe)
    {
        if (pawn?.BloodNeed() is { } n && !n.IsAnimal && n.CurBloodPoints > 0) //Animals do not have blood vials.
            if (pawn?.health?.hediffSet?.GetFirstHediff<Hediff_Alcohol>() is { } alcohol)
    ...
    {
        if (BloodItemUtility.ExtractionWillKill(pawn, BloodExtractType.Wine))
    ...
    public override void ApplyOnPawn(Pawn pawn, BodyPartRecord part, Pawn billDoer, List<Thing> ingredients, Bill bill)
    {
        ...
        if (billDoer != null)
        {
            ...
            BloodItemUtility.SpawnBloodFromExtraction(pawn, BloodExtractType.Wine);
        }
    ...
    public override string GetLabelWhenUsedOn(Pawn pawn, BodyPartRecord part)
    {
        return recipe.LabelCap + " (" + BloodTypeUtility.BloodType(pawn).GetLabel() + ")" +
               (BloodItemUtility.ExtractionWillKill(pawn, BloodExtractType.Wine)
