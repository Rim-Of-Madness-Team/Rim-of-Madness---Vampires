...
public class Recipe_ExtractBloodPack : Recipe_Surgery
{
    ...
    {
        if (BloodItemUtility.ExtractionWillKill(pawn, BloodExtractType.Pack))
            Messages.Message("ROMV_DeadlyOperation".Translate(pawn.Label), MessageTypeDefOf.NegativeEvent);
        return
            pawn.Faction !=
            billDoerFaction; //|| BloodItemUtility.ExtractionWillKill(pawn, true); //&& HealthUtility.PartRemovalIntent(pawn, part) == BodyPartRemovalIntent.Harvest;
    }
    ...
    public override void ApplyOnPawn(Pawn pawn, BodyPartRecord part, Pawn billDoer, List<Thing> ingredients, Bill bill)
    {
        ...
        if (billDoer != null)
        {
            ...
            BloodItemUtility.SpawnBloodFromExtraction(pawn, BloodExtractType.Pack);
        }
    ...
    public override string GetLabelWhenUsedOn(Pawn pawn, BodyPartRecord part)
    {
        return recipe.LabelCap + " (" + BloodTypeUtility.BloodType(pawn).GetLabel() + ")" +
               (BloodItemUtility.ExtractionWillKill(pawn, BloodExtractType.Pack)
